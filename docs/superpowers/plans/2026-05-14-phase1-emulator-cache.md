...
**Goal:** Walk all 1076 `(sim, snap)` outputs under `/scratch/cavestru_root/cavestru0/mfho/hcd_outputs/`, stack their per-class P1D + CDDF + dN/dX + parameter-vector observables into one in-repo HDF5 (`hcd_analysis/_emulator_data/observables.h5`, gitignored), interpolated onto the shared 50-bin **angular** k-grid `k_target = 2π · _DEFAULT_K_BINS` (the PRIYA convention, rad·s/km). This produces the training-ready cache that Phase 2 (the JAX two-head emulator) will read.

**Architecture:** A single Python module `scripts/build_emulator_cache.py` containing library-style functions plus a `main()` CLI entry. All work is read-only over existing on-disk files (no recomputation from raw spectra — mean-flux dimension is Phase 3, not Phase 1). The k-grid is unified via log–log linear interpolation onto `2π · hcd_analysis.p1d._DEFAULT_K_BINS` with NaN-fill for native k-ranges that don't reach the Nyquist tail. On-disk `k` in `p1d_per_class.h5` is the *cyclic* rfftfreq form (s/km); both source and target are converted to angular before interpolation so the cache lives entirely in the project-wide angular convention. The HDF5 attr `k_convention = "angular (rad*s/km), PRIYA convention"` records this. A companion test file imports the script's functions and verifies a tiny 1–2 row cache round-trip.
