...
- [ ] **Step 5: Write `scripts/train_emulator.py`** — argparse (`--cache`, `--fold`, `--epochs`, `--lr`, `--batch`, `--out`), build batches via `data.load_cache` + the splits, early-stop on a held (sim,snap) block, call `save_checkpoint`. **The encoder input is `d["x"]` (the unit-cube-normalized params⊕z from the loader — PARAM_LIMITS from PRIYA's `emulator_params.json`), NOT raw `params`** (preprocessing fix, commit e5632f4; see `docs/superpowers/2026-06-01-param-limits.md` + `...-priya-vs-ours-preprocessing.md`). Output-norm stats: fit `fit_norm` on the exact train fold, NaN-excluded, store in the checkpoint. Add a SLURM header note: run on `-A cavestru0 -p gpu --gres=gpu:1`; **profile one fold first** (cavestru0 budget is tight).
- [ ] **Step 6: Emit intermediate training figures (user-review requirement).** During/after training write to `figures/analysis/04_emulator/`: (a) `train_val_loss_fold{N}.png` — train + validation loss vs epoch (log-y); (b) `grad_norm_lr_fold{N}.png` — global grad-norm and LR schedule vs step; (c) `pred_vs_true_p1d_fold{N}.png` — predicted vs cache per-class P1D on a few held-out (sim,snap,τ₀) rows (4 class panels, log-log, residual sub-panel). Save the per-epoch loss history to `*_history.npz` so figures are reproducible. **Surface these to the user for review** (the controller sends them via the file-surface tool after each fold).
