...
1. **At the simulation/spectrum stage** via `tau_thresh` (coarse_grid.py:72, 161; flux_power.py:99): passed to `fake_spectra.get_flux_power_1D`. `_filter_tau` flags any sightline whose **maximum-pixel** τ exceeds `tau_thresh` and overwrites its saturated trough (core + wings, down to ≈τ_eff) with the effective optical depth, so δF→0 there. PRIYA production uses `tau_thresh=1e6`.
   **CORRECTION (2026-05-17, verified by two independent agents — theory + empirical):** the earlier claim here that `tau_thresh=1e6` is "effectively off" is **WRONG**. A line-center Lyα optical depth of 1e6 corresponds to log N_HI ≈ 19.1–19.6 (b=10–30 km/s); a DLA (log N_HI ≥ 20.3) has peak-pixel τ ≈ 5×10⁶–1.5×10⁷, well above it. Empirically (3 sim/snap pairs, ~1.4M absorbers): **100% of DLAs and ~56% of sub-DLAs** have a core pixel τ > 1e6. So `tau_thresh=1e6` genuinely masks the saturated cores of every DLA and the strongest sub-DLAs; it is a real DLA-strength cut, not a no-op. `fake_spectra` does not cap τ (saturated pixels reach 10⁷–10⁸). This is a coarse, single-class HCD mask — *not* a CDDF-aware four-class treatment.
