"""All-folds (60-sim) n_s/A_p EMU bias at KS z_lo=2.4 with the NEW KS k-range (drop_first4=False).

Re-confirms the coherent n_s EMU bias stays in-gate after the KS loader default flipped to
`load_ks_leg(..., drop_first4=False)` (2026-06-09): KS now keeps its FULL native k-range from
klow=0.0055 s/km (no first-4-bins drop), z_lo still 2.4, k<=0.069. These added low-k KS bins
(0.0055-0.0198) OVERLAP DESI's k-coverage; this script checks that they did NOT reintroduce the
-0.65sigma coherent n_s bias that the z_lo=2.4 cut removed.

This is IDENTICAL machinery to scripts/diag_emu_bias_allfolds_zlo24.py (the per-sim Fisher MAP-shift,
DLA mask, per-leg C_emu, bootstrap are byte-for-byte the same) except:
  * RESULTS  -> emu_bias_allfolds_zlo24_kslow.txt   (NEW file; does NOT clobber the old-KS-klow ref)
  * the figure name is suffixed _kslow
  * it also PRINTS the KS leg k-grid (sanity: confirms KS now starts at k~=0.0055)
The KS k-range change is picked up automatically: load_ks_leg now defaults drop_first4=False, and the
ks_kwargs below pass ONLY {k_max, z_lo} (NOT drop_first4), so the new full-range default is active.

Anchors:
  * old-KS-klow (drop_first4=True, klow=0.0198), z_lo=2.4, 60-sim: n_s = +0.030sigma, A_p = +0.034sigma
    (figures/analysis/04_emulator/emu_bias_allfolds_zlo24.txt)
...
RESULTS = f"{FIGDIR}/emu_bias_allfolds_zlo24_kslow.txt"   # NEW file — does NOT overwrite old-KS-klow ref
...
# drop_first4 is INTENTIONALLY not set here -> picks up the new load_ks_leg default (False, klow=0.0055)
...
def emu_bias_one_sim(ctx, d, sim, fold):
    """EMU-only A_p/n_s bias (in sigma) for one held-out sim, summed over legs. Forward at the sim's
    EXACT per-z w_c (DLA-masked) -> dP is pure emulator error."""
...
emit("# EMU bias, ALL 8 folds, each fold's OWN held-out emulator, KS z_lo=2.4, NEW KS k-range (drop_first4=False, klow=0.0055), KS<0.06. forward-only Fisher.")
emit("# Re-confirms the coherent n_s bias stays in-gate after KS keeps its full low-k range (0.0055-0.0198 bins overlap DESI).")
emit("# ANCHORS: old-KS-klow (drop_first4=True, klow=0.0198) z_lo=2.4 = n_s +0.030 / A_p +0.034 ; z_lo=2.0 ref = n_s -0.646.")
allAp, allNs, allns_u = [], [], []
ks_kgrid_printed = False
for fold in range(8):
    ...
    if not ks_kgrid_printed:
        # SANITY: print the KS leg k-grid so the new full low-k range is confirmed active.
        ks_leg = next((lg for lg in ctx.legs if lg.name == "KS"), None)
        if ks_leg is not None:
            kk = np.asarray(ks_leg.k); ku = np.unique(np.round(kk, 6))
            emit("\n# --- SANITY: KS leg k-grid (fold 0 ctx) ---")
            emit(f"#   KS legs present: {[lg.name for lg in ctx.legs]}")
            emit(f"#   KS k_min = {kk.min():.6f} s/km   k_max = {kk.max():.6f} s/km   (n rows = {kk.size}, n_z = {ks_leg.n_z})")
            emit(f"#   KS unique k bins ({ku.size}): " + ", ".join(f"{v:.4f}" for v in ku))
            emit(f"#   --> EXPECT k_min ~= 0.0055 (drop_first4=False active). {'PASS' if abs(kk.min()-0.0055) < 1e-3 else 'CHECK'}")
            emit(f"#   KS z bins: " + ", ".join(f"{z:.2f}" for z in np.asarray(ks_leg.z)))
        else:
            emit("\n# --- SANITY: NO KS leg found in ctx.legs! ---")
        ks_kgrid_printed = True
...
emit(f"\n# ===== POOLED over ALL {len(allAp)} honestly-held-out sims (KS z_lo=2.4, NEW KS klow=0.0055) =====")
...
emit("\n# ANCHOR: old-KS-klow (drop_first4=True) z_lo=2.4 n_s = +0.030sigma, A_p = +0.034sigma;")
emit("#         committed all-folds z_lo=2.0 n_s = -0.646sigma; z_lo=2.6 n_s = +0.038sigma.")
emit("#         GATE: this NEW-KS-klow z_lo=2.4 n_s & A_p must be in-gate (|bias|<0.2sigma).")
...
for col, (nm, arr) in enumerate((("A_p", allAp), ("n_s", allNs))):
    ...
    ax[col].set_title(f"{nm} EMU bias vs n_s — {len(arr)} held-out sims (all folds, KS z_lo=2.4, klow=0.0055)")
    ax[col].legend(fontsize=8); ax[col].grid(alpha=0.3)
fig.suptitle("EMU bias all folds — KS z_lo=2.4, NEW full KS k-range (drop_first4=False, klow=0.0055), KS<0.06, bootstrap CI")
p = Path(FIGDIR) / "emu_bias_allfolds_zlo24_kslow.png"; fig.savefig(p, dpi=150, bbox_inches="tight"); plt.close(fig)
emit(f"\n[fig] {p}"); emit("[done]")
