...
def load_product(cons, chain_dir, root, mode, z_lock, z_pivot):
    ...
    return dict(health=health, z=z.tolist(), cols=cols, n_draws=int(rows.shape[0]))
...
def compare(cons, full, sub):
    ...
    for k in full["cols"]:
        ...
        rec = dict(full=sa, sub=sb, delta_sub_minus_full=d,
                   delta_over_full_sd=(d / sa["sigma"] if sa["sigma"] > 0 else None),
                   delta_over_combined_sd=(d / float(np.hypot(sa["sigma"], sb["sigma"])) if (sa["sigma"] > 0 or sb["sigma"] > 0) else None),
                   sd_ratio_sub_over_full=(sb["sigma"] / sa["sigma"] if sa["sigma"] > 0 else None))
...
def to_md(res):
    L = ["# eBOSS z = 2.2 to 4.6 versus z >= 2.6 (descriptive; nested data vectors, not a tension statistic)", "",
         ...
         "| column | full median [16, 84] | sub median [16, 84] | sub minus full | / full sd | / combined sd | sd ratio | rails full (lo, hi) | rails sub (lo, hi) |",
         "|---|---|---|---|---|---|---|---|---|"]
    ...
    for k, r in res["comparison"].items():
        ...
        fu, su = r["full"], r["sub"]
        L.append("| %s | %.5g [%.5g, %.5g] | %.5g [%.5g, %.5g] | %+.4g | %s | %s | %s | %s | %s |" % (
            k, fu["median"], fu["q16"], fu["q84"], su["median"], su["q16"], su["q84"], r["delta_sub_minus_full"],
            f(r["delta_over_full_sd"]), f(r["delta_over_combined_sd"]), f(r["sd_ratio_sub_over_full"]), rails(rf), rails(rs)))
    return "\n".join(L) + "\n"
...
def main(argv=None):
    ...
    res = dict(schema="eboss_zrange_compare.v1", note="descriptive; the restricted data vector is a subset of the full one",
               ...
               dropped_z=[z for z in full["z"] if z not in sub["z"]], comparison=comp)
    ...
    print(f"wrote {a.out}.json and .md; shifts (sub minus full) in full-sd units: {key}")
    return 0
