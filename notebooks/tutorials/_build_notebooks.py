"""
Build the five student-facing tutorial notebooks under notebooks/tutorials/.
...
Style policy (applied via the ``TUTORIAL_STYLE`` and ``save_fig`` helpers
injected into every notebook's first code cell):

* Body font ≥ 12 pt, title ≥ 14 pt, axis labels ≥ 12 pt.
* Every figure has a descriptive title and axes labelled WITH UNITS.
* Multi-curve plots have a legend with class labels.
* ``plt.tight_layout()`` always before save / show.
* Every figure is saved to ``notebooks/tutorials/figures/NB<XX>_<name>.png``
  at ``dpi=200`` in addition to being displayed inline (so a reader of the
  built docs sees the figure without having to re-execute).

The figure directory is created on first save; outputs are gitignored if
the user wishes (no rule enforced here — committed pngs are reproducible
because the build script regenerates them).
"""
...
# ---------------------------------------------------------------------------
# Reusable code snippets injected into every notebook
# ---------------------------------------------------------------------------

# Style preamble. Put this as the first code cell after the imports so every
# figure in the notebook obeys the same rcParams and so that the save_fig()
# helper is in scope. Body ≥ 12 pt, title ≥ 14 pt (per project policy).
TUTORIAL_STYLE = '''
...
def style_cell():
    """Return the standard style-preamble code cell."""
    return code(TUTORIAL_STYLE.lstrip())
...
**Why look at one sightline at a time?**  The population-level
observables (CDDF, dN/dX, per-class P1D) you'll meet in notebooks 02–03
are averages over hundreds of thousands of sightlines.  Before you trust
those averages you need to know what a single absorber looks like —
this is the lowest-level "atoms of the analysis" view, and where every
bug-hunting session starts.
"""),
...
ax.hist(log_nhi, bins=np.arange(17.0, 22.0, 0.1), histtype='stepfilled',
        alpha=0.6, color='C0', edgecolor='C0', label='catalog')
for boundary, label in [(17.2, 'LLS'), (19.0, 'subDLA'), (20.3, 'DLA')]:
    ax.axvline(boundary, color='k', linestyle='--', linewidth=1)
    ax.text(boundary + 0.02, ax.get_ylim()[1] * 0.85, f'≥ {label}', fontsize=10)
ax.set_xlabel(r'$\\log_{10}\\, N_{\\rm HI}$  (cm$^{-2}$)')
ax.set_ylabel('absorbers per 0.1 dex bin')
ax.set_title(fr'Per-absorber $\\log N_{{\\rm HI}}$ distribution  (z = {meta["z"]:.2f})')
ax.set_yscale('log')
ax.legend(loc='upper right')
save_fig(fig, 'NB01_log_nhi_histogram')
plt.show()
"""),
...
fig, (ax_t, ax_f) = plt.subplots(2, 1, figsize=(9, 5.5), sharex=True)
ax_t.plot(v[sel], tau_row[sel], color='C3', lw=1.0, label=r'$\\tau(v)$')
ax_t.set_ylabel(r'optical depth $\\tau$')
ax_t.set_yscale('log')
ax_t.set_ylim(1e-2, max(2.0, tau_row[sel].max()))
ax_t.axvspan(dla.pix_start * dv, dla.pix_end * dv, color='grey', alpha=0.2,
             label='catalog pix_start..pix_end (core)')
ax_t.legend(loc='upper right')
ax_t.set_title(f'DLA on skewer {dla.skewer_idx}: '
               fr'$\\log N_{{\\rm HI}}$ = {dla.log_NHI:.2f}, $b$ = {dla.b_kms:.1f} km/s')

ax_f.plot(v[sel], F_row[sel], color='C0', lw=1.0, label=r'$F = e^{-\\tau}$')
ax_f.set_xlabel('velocity along sightline  (km/s)')
ax_f.set_ylabel(r'transmitted flux $F$')
ax_f.set_ylim(-0.05, 1.05)
ax_f.axvspan(dla.pix_start * dv, dla.pix_end * dv, color='grey', alpha=0.2,
             label='catalog pix_start..pix_end (core)')
ax_f.legend(loc='lower right')

save_fig(fig, 'NB01_one_dla_tau_F')
plt.show()
"""),
...
**Where this fits in the analysis chain.**  NB01 told you what is in the
catalog at the row level.  NB04 tells you **how** that catalog row got
there: starting from a 1D τ array, what algorithm produces a
`(skewer_idx, pix_start, pix_end, NHI)` record?  And, once a record
exists, what does "masking the HCD" actually mean numerically?  These
two questions are the substrate of every science decision downstream.
"""),
...
for col, (cls, sk, ps, pe, ln) in enumerate(EXAMPLES):
    ...
    ax_t = axes[0, col]
    ax_t.plot(v[sel], tau_row[sel], color='C3', lw=0.9)
    ax_t.set_yscale('log')
    ax_t.axvspan(ps * dv, pe * dv, color='grey', alpha=0.2,
                 label='catalog pix range' if col == 0 else None)
    ax_t.axhline(100, color='k', linestyle=':', linewidth=0.8,
                 label=r'$\\tau = 100$ (detect)' if col == 0 else None)
    ax_t.set_title(fr'{cls}: skewer {sk}, $\\log N_{{\\rm HI}}$ = {ln:.2f}')
    if col == 0:
        ax_t.set_ylabel(r'optical depth $\\tau$')
        ax_t.legend(loc='upper right', fontsize=9)

    ax_f = axes[1, col]
    ax_f.plot(v[sel], F_row[sel], color='C0', lw=0.9)
    ax_f.axvspan(ps * dv, pe * dv, color='grey', alpha=0.2)
    ax_f.set_ylim(-0.05, 1.05)
    ax_f.set_xlabel('velocity along sightline  (km/s)')
    if col == 0:
        ax_f.set_ylabel(r'transmitted flux $F = e^{-\\tau}$')

fig.suptitle(r'$\\tau(v)$ (top) and $F(v)$ (bottom): one clean example per HCD class', y=1.00)
save_fig(fig, 'NB04_class_signatures')
plt.show()
"""),
...
ax.plot(v_demo, tau_demo, color='C3', lw=0.8, label=r'$\\tau(v)$ on demo skewer')
ax.fill_between(v_demo, 1e-3, tau_demo, where=above, color='red', alpha=0.3,
                label=fr'$\\tau > {TAU_THRESHOLD:.0f}$ (above-threshold pixels)')
ax.axhline(TAU_THRESHOLD, color='k', linestyle=':', lw=0.8)
ax.set_yscale('log')
ax.set_ylim(1e-2, max(2.0, tau_demo.max() * 2))
ax.set_xlabel('velocity along sightline  (km/s)')
ax.set_ylabel(r'optical depth $\\tau$')
ax.set_title(fr'Stage 1: above-threshold pixels (skewer {DEMO_SKEWER}, $\\tau_{{\\rm thr}} = {TAU_THRESHOLD:.0f}$)')
ax.legend(loc='upper right')
save_fig(fig, 'NB04_finder_stage1_threshold')
plt.show()
"""),
...
ax.plot(v_full, tau_dla, color='C3', lw=0.9, label=r'data: $\\tau(v)$ from sim')
ax.plot(v_full, tau_model, color='k', lw=1.2, linestyle='--',
        label=fr'Voigt fit  $\\log N_{{\\rm HI}}$ = {np.log10(NHI_fit):.2f}, $b$ = {b_fit:.0f} km/s')
ax.axvspan(DLA_PS * dv, DLA_PE * dv, color='grey', alpha=0.2, label='catalog pix range (core only)')
ax.axvspan(v_lo, v_hi, color='C0', alpha=0.07, label=fr'Voigt fit window ($\\pm$ {WING_PIX} pix)')
ax.set_yscale('log')
ax.set_ylim(1e-2, max(2.0, tau_dla.max() * 2))
ax.set_xlim(v_lo - 200, v_hi + 200)
ax.set_xlabel('velocity along sightline  (km/s)')
ax.set_ylabel(r'optical depth $\\tau$')
ax.set_title(fr'DLA on skewer {DLA_SK}: $\\tau$ data and Voigt model (note damping-wing match)')
ax.legend(loc='upper right')
save_fig(fig, 'NB04_voigt_fit_one_dla')
plt.show()
"""),
...
for ax, (lbl, mk, c) in zip(axes, labels):
    ax.plot(v_full, F_dla, color='black', lw=0.7, label=r'$F(v)$')
...
fig, ax = plt.subplots(figsize=(11, 4.3))
ax.plot(v_full, F_dla, color='black', lw=0.7, label='unmasked')
for name, F in F_filled.items():
    ax.plot(v_full, F, lw=1.2, label=f'fill = {name}')
ax.axvspan(v_full[mask_tauspace].min(),
           v_full[mask_tauspace].max(),
           color='C1', alpha=0.15, label='mask region (tauspace)')
ax.set_xlim(v_centre - 2500, v_centre + 2500)
ax.set_ylim(-0.05, 1.10)
ax.set_xlabel('velocity along sightline  (km/s)')
ax.set_ylabel(r'transmitted flux $F$')
ax.set_title('Three fill strategies applied to the same tauspace mask')
ax.legend(loc='lower right')
save_fig(fig, 'NB04_three_fills')
plt.show()
"""),
